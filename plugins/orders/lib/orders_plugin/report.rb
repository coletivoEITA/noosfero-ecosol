    ...
    wb.add_worksheet(name: t('lib.report.products_report')) do |sheet|
      ...
      products_by_suppliers.each do |supplier, products, total_price_consumer_ordered|
        ...
        products.each do |product|
          ...
          stock_qty_value = product.total_quantity_consumer_ordered
          stock_formula = "=D#{pl}-C#{pl}+F#{pl}"
          stock_value = 0
          unit = product.unit.singular rescue ''
          total_price_formula = "=F#{pl}*I#{pl}"
          total_price_value = product.total_quantity_consumer_ordered * product.price

          #FIXME: correct this calc for stock
          selled_sum += total_price_value
          parcelled_sum += total_price_value

          sheet.add_row [product.id, product.name, product.total_quantity_consumer_ordered,
                         0, 0, stock_qty_formula, stock_formula,
                         unit, product.price, product.total_price_consumer_ordered, total_price_formula],
    ...
    wb.add_worksheet(name: t('lib.report.closed_orders')) do |sheet|
      ...
      orders.each do |order|
        ...
        order.items.each do |item|
          ...
          formula_value = item.product.price*item.quantity_consumer_ordered
