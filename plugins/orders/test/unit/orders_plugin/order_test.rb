...
class OrdersPlugin::OrderTest < ActiveSupport::TestCase
  ...
  should 'give total price and quantity asked' do
    ...
    @order.save!
    @order.item.create! :product => @order.cycle.products.first, :quantity_consumer_asked => 2.0
    assert_equal 2.0, @order.total_quantity_consumer_asked
    assert_equal 4.0, @order.total_price_consumer_asked
  end
