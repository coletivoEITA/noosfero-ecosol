...
module ApplicationHelper
  ...
  def usermenu_logged_in
    ...
    output += manage_enterprises_str.present? ? manage_enterprises_str  : ''

    #manage_communities
    manage_communities_str = manage_communities
    output += manage_communities_str.present? ? manage_communities_str  : ''
    ...
    (field_helpers - %w(hidden_field)).each do |selector|
      src = <<-END_SRC
        def #{selector}(field, *args, &proc)
          ...
          NoosferoFormBuilder::output_field(text, super, nil, @options)
        end
      END_SRC
