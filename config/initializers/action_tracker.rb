...
ActionTrackerConfig.verbs = {
  ...
  :create_product => {
    :description => lambda { _('create product: %{title}') % { :title => '{{ link_to(truncate(ta.get_name),ta.get_url) }}' } }
  },

  :update_product => {
    :description => lambda { _('update product: %{title}') % { :title => '{{ link_to(truncate(ta.get_name),ta.get_url) }}' } }
  },

  :remove_product => {
    :description => lambda { _('remove product: %{title}') % { :title => '{{ truncate(ta.get_name) }}' }}
 },
