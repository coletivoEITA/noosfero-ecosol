...
class ChangePasswordTest < ActiveSupport::TestCase
  ...
  should 'search for user in the correct environment' do
    ...
    c1 = ChangePassword.create!(:login => 'sample-user', :environment_id => e1.id)
    c2 = ChangePassword.create!(:login => 'sample-user', :environment_id => e2.id)
  ...
  should 'have target notification description' do
    person = create_user('testuser', :password => 'test', :password_confirmation => 'test', :email => 'testuser@test.com', :environment => Environment.default).person
